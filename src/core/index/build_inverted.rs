    ...
    pub(crate) fn build_inverted_index(
        ...
        match col_array.data_type() {
            arrow::datatypes::DataType::Int32 => {
                ...
                let inv_tmp = format!(
                    "{}.tmp",
                    inv_path.to_str().context("Invalid UTF-8 in path")?
                );
            ...
            arrow::datatypes::DataType::Int64 => {
                ...
                let inv_batch =
                    RecordBatch::try_new(inv_schema.clone(), vec![key_array, list_array])?;
                let inv_filename = format!("{}.{}.inv.parquet", self.config.segment_id, col_name);
                let inv_path = local_staging_dir.join(&inv_filename);
                let inv_tmp = format!(
                    "{}.tmp",
            ...
            arrow::datatypes::DataType::Float32 => {
                ...
                // Inverted Index
                let mut inverted_map: std::collections::HashMap<u32, Vec<u32>> =
            ...
            arrow::datatypes::DataType::Utf8 | arrow::datatypes::DataType::LargeUtf8 => {
                ...
                let casted_array =
                    arrow::compute::cast(col_array, &arrow::datatypes::DataType::Utf8)
                        .context("Failed to cast column to Utf8 for indexing")?;
                let array = casted_array
                    .as_any()
            ...
            arrow::datatypes::DataType::Date32 => {
                ...
                let mut inverted_map: std::collections::HashMap<i32, Vec<u32>> =
                    std::collections::HashMap::new();
                for (row_i, val) in array.iter().enumerate() {
                    if let Some(v) = val {
                ...
                tracing::info!(
                    "Date32 Inverted Index written to {}",
                    inv_path.to_str().context("Invalid UTF-8 in path")?
                );
            }
            ...
            arrow::datatypes::DataType::Timestamp(_, _) => {
                ...
                for row_i in 0..col_array.len() {
                    ...
                    let day = if let Some(arr) =
                        array.downcast_ref::<arrow::array::TimestampSecondArray>()
                    {
                        (arr.value(row_i) / 86_400) as i32
                    } else if let Some(arr) =
                        array.downcast_ref::<arrow::array::TimestampMillisecondArray>()
                    {
                        (arr.value(row_i) / 86_400_000) as i32
                    } else if let Some(arr) =
                        array.downcast_ref::<arrow::array::TimestampMicrosecondArray>()
                    {
                        (arr.value(row_i) / 86_400_000_000) as i32
                    } else if let Some(arr) =
                        array.downcast_ref::<arrow::array::TimestampNanosecondArray>()
                    {
                ...
                tracing::info!(
                    "Timestamp Inverted Index (day granularity) written to {}",
                    inv_path.to_str().context("Invalid UTF-8 in path")?
                );
            }
            ...
            arrow::datatypes::DataType::Boolean => {
                tracing::info!(
                    "Indexing Boolean column: {} (native boolean index)",
                    col_name
                );
                // Build inverted index: Boolean -> RowIDs (true/false as native booleans)
                let mut inverted_map: std::collections::HashMap<bool, Vec<u32>> =
                    std::collections::HashMap::new();
                ...
                tracing::info!(
                    "Boolean Inverted Index written to {}",
                    inv_path.to_str().context("Invalid UTF-8 in path")?
                );
            }
            ...
            arrow::datatypes::DataType::Time32(unit) => {
                ...
                let key_array: arrow::array::ArrayRef = match unit {
                    arrow::datatypes::TimeUnit::Second => {
                        let mut builder =
                            arrow::array::Time32SecondBuilder::with_capacity(inverted_map.len());
                        for key in &keys {
                    ...
                    arrow::datatypes::TimeUnit::Millisecond => {
                        let mut builder = arrow::array::Time32MillisecondBuilder::with_capacity(
                            inverted_map.len(),
                        );
                        for key in &keys {
                            builder.append_value(*key);
            ...
            arrow::datatypes::DataType::Time64(unit) => {
                ...
                let mut inverted_map: std::collections::HashMap<i64, Vec<u32>> =
                    std::collections::HashMap::new();
                ...
                let key_array: arrow::array::ArrayRef = match unit {
                    arrow::datatypes::TimeUnit::Microsecond => {
                        let mut builder = arrow::array::Time64MicrosecondBuilder::with_capacity(
                            inverted_map.len(),
                        );
                        for key in &keys {
                            builder.append_value(*key);
                    ...
                    arrow::datatypes::TimeUnit::Nanosecond => {
                        let mut builder = arrow::array::Time64NanosecondBuilder::with_capacity(
                            inverted_map.len(),
                        );
                        for key in &keys {
                            builder.append_value(*key);
            ...
            | arrow::datatypes::DataType::FixedSizeBinary(_) => {
                ...
                let mut inverted_map: std::collections::HashMap<Vec<u8>, Vec<u32>> =
                    std::collections::HashMap::new();
                for (row_i, val) in array.iter().enumerate() {
                    if let Some(v) = val {
                        inverted_map
            ...
            arrow::datatypes::DataType::Decimal128(precision, scale) => {
                ...
                let mut inverted_map: std::collections::HashMap<i128, Vec<u32>> =
                    std::collections::HashMap::new();
                for (row_i, val) in array.iter().enumerate() {
                    if let Some(v) = val {
                        inverted_map.entry(v).or_default().push(row_i as u32);
                ...
                let mut key_builder =
                    arrow::array::Decimal128Builder::with_capacity(inverted_map.len())
            ...
            arrow::datatypes::DataType::Dictionary(_, value_type) => {
                tracing::info!(
                    "Indexing Dictionary column: {} (unpacking to {:?})",
                    col_name,
                    value_type.as_ref()
                );
            ...
            _ => {
                // Skip unsupported
                tracing::warn!(
                    "Skipping indexing for unsupported type: {:?}",
                    col_array.data_type()
                );
            }
        }
        ...
        let (inv_path, _staging_dir) = if is_remote {
            ...
            if !base.is_empty() {
                std::fs::create_dir_all(base)
                    .context("Failed to create directory for inverted index")?;
            }
            let p = if base.is_empty() {
                ...
                format!(
                    "{}/{}.{}.inv.parquet",
                    base, self.config.segment_id, col_name
                )
            };
