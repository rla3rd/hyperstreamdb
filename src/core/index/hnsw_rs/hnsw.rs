...
#[allow(unused)]
    ...
    pub fn get_layer_iterator(&self, layer : usize) -> IterPointLayer<'_, T> {
...
impl <T:Clone+Send+Sync, D: Distance<T>+Send+Sync > Hnsw<T,D>  {
    ...
    fn search_layer(& self, point: &[T], entry_point: Arc<Point<T>> , ef:usize, layer: u8, filter: Option<&roaring::RoaringBitmap>) -> BinaryHeap<Arc<PointWithOrder<T>> > {
        //
        ...
        // Only return entry point if it matches the filter!
        let mut entry_is_valid = true;
        if let Some(f) = filter {
            if !f.contains(entry_point.origin_id as u32) {
        ...
        while candidate_points.len() > 0 {
            ...
            // f farthest point to 
            let f = return_points.peek();
            if let Some(f_pt) = f {
                assert!(f_pt.dist_to_ref >= 0.);
                assert!(c.dist_to_ref <= 0.);
                log::trace!("comparaing c : {:?} f : {:?}", -(c.dist_to_ref), f_pt.dist_to_ref);
                // IF return_points is full (>= ef), AND closest candidate is further than furthest result
                if return_points.len() >= ef && -(c.dist_to_ref) > f_pt.dist_to_ref {
                    ...
                    log::trace!("fast return from search_layer, nb points : {:?} \n \t c {:?} \n \t f {:?} dists: {:?}  {:?}", 
                                    return_points.len(), c.point_ref.p_id, f_pt.point_ref.p_id, -(c.dist_to_ref), f_pt.dist_to_ref);
                    return return_points;
                }
            }
            // We already have the if-check logic replacing fast return above!
            ...
            for e in neighbours_c_l {
                ...
                if visited_point_id.contains_key(&e.point_ref.p_id) != true {
                    ...
                    if let Some(f) = filter {
                        if !f.contains(e.point_ref.origin_id as u32) {
                            // Does not match filter, CANNOT be a return point
                            enters_return = false;
                        } else {
                            if return_points.len() < ef {
                                enters_return = true;
                            } else if let Some(f_pt) = return_points.peek() {
                                if e_dist_to_p < f_pt.dist_to_ref {
                                    enters_return = true;
                                }
                            }
                        }
                    ...
                    // For exploring, we use the graph traversal logic!
                    // Is this node promising enough to explore?
                    let is_promising = if return_points.len() < ef {
                        true
                    } else if let Some(f_pt) = return_points.peek() {
                        e_dist_to_p < f_pt.dist_to_ref
                    } else {
                        true
                    };
   ...
   pub fn insert_slice(&self, data_with_id: (&[T],usize))  {
    ...
    pub fn search(&self, data :&[T] , knbn:usize, ef_arg:usize, filter: Option<&roaring::RoaringBitmap>) -> Vec<Neighbour> {
        //
        let entry_point;
        {  // a lock on an option an a Arc<Point>
            let entry_point_opt_ref = self.layer_indexed_points.entry_point.read();
    ...
    fn search_with_id(&self, request : (usize, &Vec<T>) , knbn:usize, ef:usize) -> (usize, Vec<Neighbour>) {
        (request.0, self.search(request.1, knbn, ef, None))
    }
