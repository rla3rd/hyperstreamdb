...
impl Table {
    ...
    pub async fn add_column(
        ...
        let mut current_schema = if let Some(schema) = manifest
            .schemas
            .iter()
            .find(|s| s.schema_id == manifest.current_schema_id)
        {
            ...
            crate::core::manifest::Schema::new(
                0,
                arrow_schema
                    .fields()
                    .iter()
                    .enumerate()
                    .map(|(i, f)| crate::core::manifest::SchemaField {
                        id: i as i32 + 1,
                        name: f.name().clone(),
        ...
        manifest_manager
            .update_schema(manifest.schemas, manifest.current_schema_id, Some(new_id))
            .await?;
        tracing::info!(
            "Schema Evolution: Added column '{}' (Schema ID: {})",
            name,
            new_schema_id
        );
    ...
    pub async fn update_spec(
        ...
        tracing::info!(
            "Partition Evolution: New spec ID {} with {} fields",
            new_spec_id,
            fields.len()
        );
    ...
    pub async fn drop_column(&self, name: &str) -> Result<()> {
        ...
        let mut current_schema = manifest
            .schemas
            .iter()
            .find(|s| s.schema_id == manifest.current_schema_id)
            .ok_or_else(|| anyhow::anyhow!("No active schema found for evolution"))?
            .clone();
        ...
        manifest_manager
            .update_schema(
                manifest.schemas,
                manifest.current_schema_id,
                Some(manifest.last_column_id),
            )
            .await?;
        tracing::info!(
            "Schema Evolution: Dropped column '{}' (Schema ID: {})",
            name,
            new_schema_id
        );
    ...
    pub async fn rename_column(&self, old_name: &str, new_name: &str) -> Result<()> {
        ...
        if let Some(field) = current_schema
            .fields
            .iter_mut()
        ...
        manifest_manager
            ...
            .await?;
        tracing::info!(
            "Schema Evolution: Renamed '{}' -> '{}' (Schema ID: {})",
            old_name,
            new_name,
            new_schema_id
        );
    ...
    pub async fn update_column_type(&self, name: &str, new_type: &str) -> Result<()> {
        ...
        manifest_manager
            ...
            .await?;
        tracing::info!(
            "Schema Evolution: Updated column type '{}' to '{}' (Schema ID: {})",
            name,
            new_type,
            new_schema_id
        );
    ...
    fn can_promote(old_type: &str, new_type: &str) -> bool {
        match (
            old_type.to_lowercase().as_str(),
            new_type.to_lowercase().as_str(),
        ) {
            ("int" | "int32", "long" | "int64") => true,
            ("float" | "float32", "double" | "float64") => true,
            (o, n)
                if (o.contains("decimal") || o.contains("decimal"))
                    && (n.contains("decimal") || n.contains("decimal")) =>
            {
    ...
    pub async fn move_column(&self, name: &str, new_index: usize) -> Result<()> {
        ...
        manifest_manager
            ...
            .await?;
        tracing::info!(
            "Schema Evolution: Moved column '{}' to index {} (Schema ID: {})",
            name,
            new_index,
            new_schema_id
        );
