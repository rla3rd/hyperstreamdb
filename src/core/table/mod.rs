...
use crate::core::manifest::{
    IndexAlgorithm, Manifest, ManifestEntry, ManifestManager, NullOrder, PartitionSpec,
...
impl Table {
            ...
            for (col, algs) in &column_indexes {
                if algs.is_empty() {
                    ...
                    let config =
                        index_configs
                            .entry(col.clone())
                            .or_insert_with(|| ColumnIndexConfig {
                                enabled: true,
    ...
    async fn _validate_pk_uniqueness(
        &self,
        field_ids: &[i32],
    ...
    async fn _check_pk_in_storage_async(
        &self,
        column: &str,
    ...
    pub async fn recover_indexes_async(&self) -> Result<usize> {
        ...
        let unindexed_count = all_entries.iter().filter(|e| e.index_files.is_empty()).count();
        if unindexed_count == 0 {
            tracing::info!("All segments have valid overlay indexes; no recovery needed.");
            return Ok(0);
        }

        tracing::info!("Recovering overlay indexes for {} unindexed/compacted segments...", unindexed_count);
...
impl Table {
    ...
    pub async fn read_file_async(
        ...
        if !index_used {
            ...
            if let Some(filter_str) = filter {
                let filter_expr_owned = Arc::new(
                    crate::core::planner::FilterExpr::parse_sql(filter_str, self.arrow_schema())
                        .await?,
                );
                            ...
                            Ok(b) => {
                                match planner.filter_expr(&b, &filter_expr_cloned) {
                                    ...
                                    Err(e) => {
                                        ...
                                        Some(Ok::<arrow::record_batch::RecordBatch, anyhow::Error>(
                                            b,
                                        )) // fallback to returning the batch on error
                                    }
                                }
                            }
                            Err(e) => Some(Err(e)),
    ...
    async fn shuffle_batch_by_centroids(
        ...
        if n < 1024 {
            return Ok(batch.clone());
        } // Too small to benefit from shuffling
