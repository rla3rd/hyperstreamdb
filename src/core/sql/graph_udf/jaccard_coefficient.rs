...
impl PartialEq for JaccardCoefficientUDF {
    fn eq(&self, _other: &Self) -> bool {
        true
    }
}
impl Eq for JaccardCoefficientUDF {}
impl std::hash::Hash for JaccardCoefficientUDF {
...
impl AggregateUDFImpl for JaccardCoefficientUDF {
    ...
    fn accumulator(&self, _arg: AccumulatorArgs) -> Result<Box<dyn Accumulator>> {
        Ok(Box::new(JaccardAccumulator::new()))
    }
...
impl Accumulator for JaccardAccumulator {
    ...
    fn evaluate(&mut self) -> Result<ScalarValue> {
        ...
        let union_size = start_neighbors.union(&end_neighbors).count() as f64;

        if union_size == 0.0 {
            return Ok(ScalarValue::Float64(Some(0.0)));
        }

        let intersection_size = start_neighbors.intersection(&end_neighbors).count() as f64;

        Ok(ScalarValue::Float64(Some(intersection_size / union_size)))
    }
