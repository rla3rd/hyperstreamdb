...
#[derive(Debug, Clone)]
pub struct ResourceAllocationUDF {
    signature: Signature,
}

impl PartialEq for ResourceAllocationUDF {
    fn eq(&self, _other: &Self) -> bool {
        true
    }
}
impl Eq for ResourceAllocationUDF {}
impl std::hash::Hash for ResourceAllocationUDF {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        std::any::type_name::<Self>().hash(state);
    }
}

impl Default for ResourceAllocationUDF {
    fn default() -> Self {
        Self::new()
    }
}

impl ResourceAllocationUDF {
    pub fn new() -> Self {
        Self {
...
impl AggregateUDFImpl for ResourceAllocationUDF {
    ...
    fn accumulator(&self, _arg: AccumulatorArgs) -> Result<Box<dyn Accumulator>> {
        Ok(Box::new(ResourceAllocationAccumulator::new()))
    }
...
#[derive(Debug)]
pub struct ResourceAllocationAccumulator {
    sources: Vec<u64>,
    targets: Vec<u64>,
...
impl ResourceAllocationAccumulator {
    fn new() -> Self {
        Self {
            sources: Vec::new(),
            targets: Vec::new(),
...
impl Accumulator for ResourceAllocationAccumulator {
