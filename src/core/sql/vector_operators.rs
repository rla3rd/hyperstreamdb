...
mod tests {
    ...
    fn test_operator_uniqueness() {
        ...
        for mapping in VECTOR_OPERATORS {
            assert!(
                operators.insert(mapping.operator),
                "Duplicate operator found: {}",
                mapping.operator
            );
        }
    ...
    fn test_all_operators_have_descriptions() {
        ...
        for mapping in VECTOR_OPERATORS {
            assert!(
                ...
                "Operator {} has empty description",
                mapping.operator
            );
            assert!(
                mapping.description.len() > 5,
                "Operator {} has too short description",
                mapping.operator
            );
        }
    }
