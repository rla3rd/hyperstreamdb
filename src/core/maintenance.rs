...
impl Maintenance {
    ...
    pub async fn expire_snapshots(&self, retain_last: usize) -> Result<()> {
        ...
        if history.len() <= retain_last {
            tracing::debug!(
                "History length ({}) <= retain_last ({}). No action.",
                history.len(),
                retain_last
            );
            return Ok(());
        ...
        tracing::info!(
            "Expiring {} snapshots. Retaining latest {}.",
            expired.len(),
            retained.len()
        );
...
mod tests {
    ...
    async fn test_maintenance_flow() -> Result<()> {
        ...
        let entries: Vec<_> = std::fs::read_dir(&temp_dir)?
            ...
            .collect();
        assert!(
            !entries.contains(&"seg_a.parquet".to_string()),
            "seg_a should be deleted"
        );
        assert!(
            entries.contains(&"seg_b.parquet".to_string()),
            "seg_b should exist"
        );
        assert!(
            entries.contains(&"seg_c.parquet".to_string()),
            "seg_c should exist"
        );
