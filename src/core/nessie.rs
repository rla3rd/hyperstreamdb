...
impl NessieClient {
    ...
    pub async fn resolve_reference(&self, ref_name: &str) -> Result<String> {
        ...
        let resp = builder
            .send()
            .await
            .context("Failed to connect to Nessie")?;
        ...
        let json: serde_json::Value = resp
            .json()
            .await
            .context("Failed to parse Nessie response")?;
    ...
    pub async fn load_table_metadata(
        ...
        let reference = if let Some(h) = &ref_hash {
            h
        } else {
            &self.config.ref_name
        };
        ...
        let mut builder = self.client.get(&url).query(&[("key", &key)]);
        ...
        let resp = builder
            .send()
            .await
            .context("Failed to fetch table content from Nessie")?;
        ...
        if let Some(content) = json
            ...
            .and_then(|a| a.first())
        {
            if let Some(meta_loc) = content
                .get("content")
                .and_then(|c| c.get("metadataLocation"))
                .and_then(|v| v.as_str())
            {
        ...
        anyhow::bail!(
            "Table {} not found or missing metadataLocation in Nessie response",
            key
        );
    }
}
