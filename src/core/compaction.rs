...
impl Compactor {
    ...
    pub async fn rewrite_data_files(&self) -> Result<()> {
        ...
        tracing::info!(
            "Starting compaction with strategy: {}",
        ...
        if !all_new_entries.is_empty() {
            tracing::info!(
                "Committing Batch: +{} entries, -{} paths",
                all_new_entries.len(),
                all_old_paths.len()
            );
            self.manifest
...
mod tests {
    ...
    async fn test_zorder_clustering() -> Result<()> {
        ...
        for batch in results {
            ...
            assert_eq!(rx.value(0), 0);
            assert_eq!(ry.value(0), 0);
            assert_eq!(rx.value(1), 0);
            assert_eq!(ry.value(1), 1);
            assert_eq!(rx.value(2), 1);
            assert_eq!(ry.value(2), 0);
            assert_eq!(rx.value(3), 1);
            assert_eq!(ry.value(3), 1);
        }
    ...
    async fn test_hilbert_clustering() -> Result<()> {
        ...
        for batch in results {
            ...
            assert_eq!(rx.value(0), 0);
            assert_eq!(ry.value(0), 0);
            assert_eq!(rx.value(1), 1);
            assert_eq!(ry.value(1), 0);
            assert_eq!(rx.value(2), 1);
            assert_eq!(ry.value(2), 1);
            assert_eq!(rx.value(3), 0);
            assert_eq!(ry.value(3), 1);
        }
