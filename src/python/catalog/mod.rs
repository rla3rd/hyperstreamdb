...
pub fn create_catalog(
    ...
    match catalog_type.to_lowercase().as_str() {
        "nessie" => {
            let url = config.get("url").cloned().ok_or_else(|| {
                PyErr::new::<pyo3::exceptions::PyValueError, _>("Missing 'url' for Nessie catalog")
            })?;
        ...
        "rest" => {
            let url = config.get("url").cloned().ok_or_else(|| {
                PyErr::new::<pyo3::exceptions::PyValueError, _>("Missing 'url' for Rest catalog")
            })?;
        ...
        "hive" => {
            let url = config.get("url").cloned().ok_or_else(|| {
                PyErr::new::<pyo3::exceptions::PyValueError, _>("Missing 'url' for Hive catalog")
            })?;
        ...
        "unity" => {
            let url = config.get("url").cloned().ok_or_else(|| {
                PyErr::new::<pyo3::exceptions::PyValueError, _>("Missing 'url' for Unity catalog")
            })?;
            let token = config.get("token").cloned().ok_or_else(|| {
                PyErr::new::<pyo3::exceptions::PyValueError, _>("Missing 'token' for Unity catalog")
            })?;
            let catalog = PyUnityCatalog::new(url, token)?;
            Ok(Py::new(py, catalog)?.into_any())
        }
        "jdbc" => {
            let uri = config.get("uri").cloned().ok_or_else(|| {
                PyErr::new::<pyo3::exceptions::PyValueError, _>("Missing 'uri' for JDBC catalog")
            })?;
...
pub fn create_catalog_from_config(py: Python<'_>, path: String) -> PyResult<Py<PyAny>> {
    ...
    match config.catalog_type {
        CatalogType::Nessie => {
            let url = config.config.get("url").cloned().ok_or_else(|| {
                PyErr::new::<pyo3::exceptions::PyValueError, _>("Missing 'url' for Nessie catalog")
            })?;
            let catalog = PyNessieCatalog::new(url)?;
            Ok(Py::new(py, catalog)?.into_any())
        ...
        CatalogType::Unity => {
            ...
            let token = config.config.get("token").cloned().ok_or_else(|| {
                PyErr::new::<pyo3::exceptions::PyValueError, _>("Missing 'token' for Unity catalog")
            })?;
            let catalog = PyUnityCatalog::new(url, token)?;
            Ok(Py::new(py, catalog)?.into_any())
        ...
        CatalogType::Jdbc => {
            let uri = config.config.get("uri").cloned().ok_or_else(|| {
                PyErr::new::<pyo3::exceptions::PyValueError, _>("Missing 'uri' for JDBC catalog")
            })?;
