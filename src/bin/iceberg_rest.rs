...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route(
            ...
            get(get_table).post(update_table),
        )
        .route(
            "/v1/:prefix/namespaces/:namespace/tables/:table/register",
            post(register_table),
        )
...
async fn get_table(
    ...
    let version = if let Ok(res) = store
...
async fn update_table(
    ...
    let mut all_existing_entries = if payload.updates.iter().any(|u| {
        matches!(
            ...
            TableUpdateAction::RemoveSidecarIndex { .. }
                | TableUpdateAction::AddSidecarIndex { .. }
        )
    }) {
        Some(
            manager
                .load_all_entries(&current_manifest)
                .await
                .unwrap_or_default(),
        )
    } else {
    ...
    for action in payload.updates {
        match action {
            TableUpdateAction::AddSidecarIndex {
                ...
                if let Some(entries) = &mut all_existing_entries {
                    if let Some(entry) = entries.iter_mut().find(|e| e.file_path == file_path) {
                        ...
                        println!(
                            "Warning: Data file not found for index attachement: {}",
                            file_path
                        );
                    }
                }
                    ...
                    if let Ok(get_res) = res {
                            ...
                            hyperstreamdb::core::iceberg::read_manifest_list(&bytes[..])
                        {
                            ...
                            for entry in list {
                                ...
                                let clean_path = if !root_path_str.is_empty()
                                    && path_no_scheme.starts_with(root_path_str)
                                {
                                    path_no_scheme
                                ...
                                match store.get(&m_path).await {
                                    Ok(m_res) => {
                                            ...
                                            )
                                        {
                                            ...
                                            // Convert entries
                                            let schema = current_manifest
                                                .schemas
                                                .last()
                                                .or(current_manifest.schemas.first());
                                            ...
                                            if let Some(s) = schema {
                                                ...
                                                println!(
                                                    "Warning: No schema available to decode stats"
                                                );
                                            }
                                        } else {
                                            println!(
                                                "Failed to parse Avro manifest: {}",
                                                clean_path
                                            );
                                        }
                                    }
            ...
            TableUpdateAction::AddPartitionSpec { spec } => {
                let mut specs = updated_partition_specs
                    .clone()
                    .unwrap_or_else(|| current_manifest.partition_specs.clone());
            ...
            TableUpdateAction::SetProperties { updates } => {
                let mut props: std::collections::HashMap<String, String> =
                    updated_properties.take().unwrap_or_default();
            ...
            TableUpdateAction::AddSortOrder { sort_order } => {
                let mut orders = updated_sort_orders
                    .clone()
            ...
            _ => {
                println!("Ignoring update action: {:?}", action);
            }
        }
    }
    ...
    match manager.commit(&new_entries, &[], commit_metadata).await {
        Ok(_) => {
            ...
            get_table(ax_lib::extract::Path((prefix, namespace, table)))
                .await
                .into_response()
        }
        Err(e) => {
...
async fn register_table(
    ...
    println!(
        "Registering table {}.{}.{} with payload: {:?}",
        prefix, namespace, table, payload
    );
