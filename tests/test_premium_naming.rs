...
async fn test_excel_naming_logic() -> anyhow::Result<()> {
    ...
    assert!(
        updated_schema.fields().iter().any(|f| f.name() == "A"),
        "Field 'A' not found"
    );
    assert!(
        updated_schema.fields().iter().any(|f| f.name() == "B"),
        "Field 'B' not found"
    );
...
async fn test_polars_naming_logic() -> anyhow::Result<()> {
    ...
    // 1. Create Data
    let schema = Arc::new(Schema::new(vec![Field::new("0", DataType::Int32, false)]));
    ...
    assert!(
        updated_schema
            .fields()
            .iter()
            .any(|f| f.name() == "column_1"),
        "Field 'column_1' not found"
    );
