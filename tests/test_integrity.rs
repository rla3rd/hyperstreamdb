...
async fn test_integrity_validation() -> Result<()> {
    ...
    assert!(initial_verify.is_ok(), "Data integrity should be valid after normal write, but got error: {:?}", initial_verify.err().unwrap());
    ...
    full_path.push(segments[0].file_path.strip_prefix("file://").unwrap_or(&segments[0].file_path));
    ...
    assert!(result.is_err(), "Data integrity should fail after tampering");
    
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.contains("Data integrity validation failed"), "Unexpected error message: {}", err_msg);
