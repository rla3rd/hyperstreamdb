...
use arrow::array::{
    ...
    LargeBinaryArray, ListArray, StringArray, StructArray, Time64MicrosecondArray,
...
async fn test_temporal_types() -> Result<()> {
    ...
    let batch = RecordBatch::try_new(
        ...
        vec![
            ...
            Arc::new(
                TimestampMicrosecondArray::from(vec![1_609_459_200_000_000]).with_timezone("UTC"),
            ),
        ],
    )?;
    ...
    assert!(schema_msg
        .fields
        .iter()
        .any(|f| f.name == "f_ts_tz" && f.type_str.contains("timestamp")));
...
async fn test_schema_evolution_full_lifecycle() -> Result<()> {
    ...
    let table = Table::new_async(uri.clone()).await?;
    assert_eq!(
        table.arrow_schema().field_with_name("age")?.data_type(),
        &DataType::Int64
    );
...
async fn test_nested_schema_and_field_id_preservation() -> Result<()> {
    ...
    let schema_field = manifest.schemas[0]
        .fields
        .iter()
        .find(|f| f.name == "info")
        .unwrap();
...
async fn test_partition_transforms() -> Result<()> {
    ...
    assert!(
        stats.file_count >= 2,
        "Expected at least 2 partition files, got {}",
        stats.file_count
    );
