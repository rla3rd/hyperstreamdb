...
async fn test_pgvector_operators_use_gpu_context() {
    ...
    let batch = datafusion::arrow::record_batch::RecordBatch::try_from_iter(vec![
        (
            "v1",
            Arc::new(list1) as Arc<dyn datafusion::arrow::array::Array>,
        ),
        (
            "v2",
            Arc::new(list2) as Arc<dyn datafusion::arrow::array::Array>,
        ),
    ])
    .unwrap();
    ...
    let result_cpu = df
        .clone()
        .select(vec![Expr::ScalarFunction(
    ...
    let distance_array_cpu = result_cpu[0]
        .column(0)
        .as_any()
    ...
    assert!(
        (cpu_distance - 5.196).abs() < 0.01,
        "CPU distance should be approximately 5.196, got {}",
        cpu_distance
    );
    ...
    let distance_array_gpu = result_gpu[0]
        .column(0)
        .as_any()
        .downcast_ref::<Float32Array>()
        .unwrap();
    ...
    assert!(
        (gpu_distance - cpu_distance).abs() < 0.001,
        "GPU and CPU distances should match, got CPU: {}, GPU: {}",
        cpu_distance,
        gpu_distance
    );
...
async fn test_all_pgvector_operators_with_gpu() {
    ...
    for (udf_name, operator, metric_name) in operators {
        ...
        assert!(
            result.is_ok(),
            "Operator {} ({}) should compute successfully with GPU context",
            operator,
            metric_name
        );
        ...
        assert!(
            distance.is_finite(),
            "{} distance (operator {}) should be finite, got {}",
            metric_name,
            operator,
            distance
        );
...
async fn test_gpu_context_persistence_across_queries() {
    ...
    for _ in 0..3 {
        ...
        assert!(
            result.is_ok(),
            "Query should succeed with persistent GPU context"
        );
...
async fn test_pgvector_operators_batch_with_gpu() {
    ...
    let field = Arc::new(Field::new("item", DataType::Float32, true));
    let list_array =
        FixedSizeListArray::try_new(field.clone(), 3, Arc::new(values_array), None).unwrap();
    ...
    let batch = datafusion::arrow::record_batch::RecordBatch::try_from_iter(vec![
        (
            "embedding",
            Arc::new(list_array) as Arc<dyn datafusion::arrow::array::Array>,
        ),
        (
            "query",
            Arc::new(query_list) as Arc<dyn datafusion::arrow::array::Array>,
        ),
    ])
    .unwrap();
    ...
    assert_eq!(
        distance_array.len(),
        4,
        "Should compute distances for all rows"
    );
    ...
    for i in 0..4 {
        ...
        assert!(
            dist.is_finite(),
            "Distance for row {} should be finite, got {}",
            i,
            dist
        );
    }
    ...
    assert!(
        distance_array.value(0).abs() < 0.001,
        "Distance from [1,0,0] to [1,0,0] should be ~0, got {}",
        distance_array.value(0)
    );
