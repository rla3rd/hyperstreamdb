...
async fn test_index_lifecycle_add_drop_readd() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        1,
        "Should only have ONE segment in manifest"
    );
    assert!(
        !entry.index_files.is_empty(),
        "Index files should be registered in manifest"
    );
    assert!(
        entry
            .index_files
    ...
    let (results, _) = session
        .sql("SELECT * FROM docs WHERE text = 'database'")
        .await?;
    assert!(
        !results.is_empty(),
        "Should find results after adding index"
    );
    ...
    let text_field = latest_schema
        .fields
        .iter()
        .find(|f| f.name == "text")
        .unwrap();
    let cat_field = latest_schema
        .fields
        .iter()
