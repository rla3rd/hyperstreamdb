...
fn test_jaccard_distance_cpu() -> Result<()> {
    ...
    assert!(
        (distances[1] - (1.0 / 3.0)).abs() < 1e-5,
        "Expected 0.333, got {}",
        distances[1]
    );
    assert!(
        (distances[2] - 1.0).abs() < 1e-5,
        "Expected 1.0, got {}",
        distances[2]
    );
    assert!(
        (distances[3] - 1.0).abs() < 1e-5,
        "Expected 1.0 (no overlap), got {}",
        distances[3]
    );
...
fn test_jaccard_distance_edge_cases() -> Result<()> {
    ...
    assert!(
        (distances[0] - 0.0).abs() < 1e-5,
        "All zeros should give distance 0.0"
    );
