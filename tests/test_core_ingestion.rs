...
async fn test_core_ingestion_buffered_and_wal() -> anyhow::Result<()> {
    ...
    assert_eq!(
        table2.write_buffer_row_count(),
        2,
        "Should have recovered 2 rows from WAL"
    );

    // Verify memory index was also re-built during recovery
    assert!(
        table2.has_memory_index(),
        "Memory index should be re-built from WAL"
    );
    ...
    println!(
        "Manifest version: {}, sharded: {}",
        manifest.version,
        manifest.manifest_list_path.is_some()
    );
...
async fn test_recovered_evolved_wal() -> anyhow::Result<()> {
    ...
    // Reopen and Recover everything
    {
        ...
        let name_col_exists = batches
            .iter()
            .any(|b| b.schema().field_with_name("name").is_ok());
    ...
    // 4. Test "Narrower" batch correctly promotes to widest schema
    {
        ...
        let schema_thin = Arc::new(Schema::new(vec![Field::new(
            "id",
            arrow::datatypes::DataType::Int32,
            false,
        )]));
        let batch_thin = RecordBatch::try_new(
            schema_thin.clone(),
        ...
        let total_name_nulls: usize = batches
            .iter()
            .map(|b| {
