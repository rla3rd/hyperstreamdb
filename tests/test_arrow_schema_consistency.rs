...
async fn test_vector_search_schema_consistency() -> anyhow::Result<()> {
    ...
    // 3. Test Normal Search Result Schema
    {
        ...
        assert!(
            result_schema.field_with_name("distance").is_ok(),
            "Schema should contain 'distance' column"
        );
        assert_eq!(
            result_schema
    ...
    // 5. Test Projection with Vector Search
    {
        ...
        assert!(
            result_schema.field_with_name("distance").is_ok(),
            "Distance should be present even if not explicitly selected"
        );
        assert!(
            result_schema.field_with_name("embedding").is_err(),
            "Embedding should NOT be present if not selected"
        );
    }
