...
async fn test_string_identity_partitioning() -> anyhow::Result<()> {
    ...
    // 2. Create Table
    let table =
        Table::create_partitioned_async(uri.clone(), arrow_schema.clone(), spec.clone()).await?;
    ...
    let dict_table =
        Table::create_partitioned_async(dict_uri.clone(), dict_schema.clone(), spec).await?;
